...
export class SchedulerPage extends PageBase {
    ...
    resourceLabelDidMount(arg) {
        ...
        arg.el.querySelector('.del-event-btn').onclick = function (e) {
            e.preventDefault();
            e.stopPropagation()
            that.env.showPrompt('Bạn chắc muốn xóa nhân sự này?', null, 'Phân ca')
                .then(_ => {
                    that.submitAttempt = true;
                    console.log(arg);

                    that.staffTimesheetEnrollmentProvider.delete([{ Id: parseInt(arg.resource._resource.extendedProps.Id) }])
                        .then((savedItem: any) => {
                            arg.resource.remove();
    ...
    eventChange(changeInfo) {
        ...
        this.pageProvider.save(ev).then(_ => {
            this.env.showTranslateMessage('erp.app.pages.hrm.scheduler.message.update-shift-complete','warning');
        })
