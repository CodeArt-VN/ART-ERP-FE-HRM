...
export class PersonalSchedulerGeneratorPage extends PageBase {
    ...
    massShiftAssignment() {
        ...
        if (!this.formGroup.valid) {
            this.env.showTranslateMessage('erp.app.pages.hrm.personal-scheduler.message.check-red-above','warning');
        }
        else {
            let submitItem = this.formGroup.value;//this.getDirtyValues(this.formGroup);
            this.env.showTranslateMessage('erp.app.pages.hrm.personal-scheduler.message.save-info-complete','success');
            this.modalController.dismiss(submitItem);
        }
    }
}
