...
export class CasualLabourRegisterPage extends PageBase {
    ...
    changeLock() {
        ...
        if (this.userAccount.LockoutEnabled) {
            this.env.showTranslateMessage('erp.app.pages.hrm.casual-labour-register.message.account-locked','warning');
        }
        else {
            this.env.showTranslateMessage('erp.app.pages.hrm.casual-labour-register.message.account-normal','warning');
        }
    ...
    staffSearch() {
        ...
        this.staffList$ = concat(
            ...
            this.staffListInput$.pipe(
                ...
                switchMap(term => this.pageProvider.search({ Take: 20, Skip: 0, Term: term }).pipe(
