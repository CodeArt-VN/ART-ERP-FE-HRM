...
export class StaffPersonnelProfileComponent extends PageBase {
				...
				this.urserProvider
					...
					.catch((err) => {
						...
						if (err.error?.ExceptionMessage) {
							if (err.error.ExceptionMessage.includes('Name') || err.error.ExceptionMessage.includes('Email')) this.env.showMessage('Email has already been used for account registration, please check again', 'danger');
							else this.env.showMessage(err.error.ExceptionMessage, 'danger');	
	...
	async resetPassword() {
		if (!this.changePasswordForm.valid) {
			...
			await loading.present().then(() => {
				this.urserProvider
					.resetPassword(this.userAccount.Id, this.changePasswordForm.controls.newPassword.value, this.changePasswordForm.controls.confirmPassword.value)
					.then((savedItem: any) => {
