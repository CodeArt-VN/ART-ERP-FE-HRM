...
export class SchedulerGeneratorPage extends PageBase {
    ...
    massShiftAssignment() {
        if(!this.formGroup.controls.IDShift.value && !this.formGroup.controls.TimeOffType.value){
            this.env.showTranslateMessage('erp.app.pages.hrm.scheduler-generator.message.select-shift-or-sub-type','danger', null, 0, true);
            return;
        }

        this.formGroup.updateValueAndValidity();
        if (!this.formGroup.valid) {
            this.env.showTranslateMessage('erp.app.pages.hrm.scheduler-generator.message.check-red-above','warning');
        }
        else {
