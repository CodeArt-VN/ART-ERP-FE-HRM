...
@Component({
	selector: 'app-staff-picker-enrollment',
	templateUrl: 'staff-picker-enrollment.page.html',
	styleUrls: ['staff-picker-enrollment.page.scss'],
	standalone: false,
})
export class StaffPickerEnrollmentPage extends PageBase {
	branchList = [];
	departmentList = [];
	quickSelect = {
		IDDepartment: -1,
	...
	SaveSelectedStaff() {
		this.modalController.dismiss({ IDPol: this.idPol, StaffList: this.selectedItems.map((d) => ({ IDStaff: d.Id, _Staff:d })) });
	}
